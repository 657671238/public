...
class datset(Dataset):
    ...
    def __init__(self, root_dir, train=True, size=14):
        """
        Args:
    ...
    def __getitem__(self, idx):
        image = self.images[idx + self.start]
        noise = self.noises[idx + self.start]
        ...
        if self.train:
            noise,image = make_augment(noise, image)
            #print(rand_mode,image.shape,noise.shape)

        noise,image = cut_img(noise, image)
        image = modcrop(image, self.size)
        noise = modcrop(noise, self.size)
